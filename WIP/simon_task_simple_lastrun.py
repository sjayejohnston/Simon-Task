...
expName = 'simon_task_simple'  # from the Builder filename that created this script
expInfo = {
    'session': '001',
    'participant': '',
    'counterbalance': ['0','1'],
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\steph\\Downloads\\simonTaskTutorial-master\\simonTaskTutorial-master\\simon_task_simple_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    # Run 'Begin Experiment' code from InitCode
    counterbalance=expInfo['counterbalance']
    
    # --- Initialize components for Routine "prac_instructions_CB0" ---
    ...
    # --- Initialize components for Routine "prac_instrucations_CB1" ---
    text_norm_cb1 = visual.TextStim(win=win, name='text_norm_cb1',
        text="Basic Simon Task\n\nOnscren you will seee either a 'O' or an 'X' as a target. Your task is to identify the target.\n\nIf you see an 'X' press 'z' on the keyboard\nIf you see an 'O' press 'm' on the keyboard\n\nThe first 10 trials that follow this screen are practice trials.\n\nPress the spacebar to continue",
        font='Open Sans',
    ...
    # --- Prepare to start Routine "prac_instructions_CB0" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('prac_instructions_CB0.started', globalClock.getTime())
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (expInfo['counterbalance']==0)
    key_instruct.keys = []
    key_instruct.rt = []
    _key_instruct_allKeys = []
    # keep track of which components have finished
    prac_instructions_CB0Components = [text_norm, key_instruct]
    for thisComponent in prac_instructions_CB0Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "prac_instructions_CB0" ---
    routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
    ...
    # --- Ending Routine "prac_instructions_CB0" ---
    for thisComponent in prac_instructions_CB0Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('prac_instructions_CB0.stopped', globalClock.getTime())
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "prac_instructions_CB0" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    
    # --- Prepare to start Routine "prac_instrucations_CB1" ---
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('prac_instrucations_CB1.started', globalClock.getTime())
    # skip this Routine if its 'Skip if' condition is True
    continueRoutine = continueRoutine and not (expInfo['counterbalance']==1)
    key_resp.keys = []
    key_resp.rt = []
    _key_resp_allKeys = []
    # keep track of which components have finished
    prac_instrucations_CB1Components = [text_norm_cb1, key_resp]
    for thisComponent in prac_instrucations_CB1Components:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    # --- Run Routine "prac_instrucations_CB1" ---
    routineForceEnded = not continueRoutine
    while continueRoutine:
        # get current time
        t = routineTimer.getTime()
        ...
        # *text_norm_cb1* updates
        
        # if text_norm_cb1 is starting this frame...
        if text_norm_cb1.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            # keep track of start time/frame for later
            text_norm_cb1.frameNStart = frameN  # exact frame index
            text_norm_cb1.tStart = t  # local t and not account for scr refresh
            text_norm_cb1.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(text_norm_cb1, 'tStartRefresh')  # time at next scr refresh
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'text_norm_cb1.started')
            # update status
            text_norm_cb1.status = STARTED
            text_norm_cb1.setAutoDraw(True)
        
        # if text_norm_cb1 is active this frame...
        if text_norm_cb1.status == STARTED:
            # update params
            pass
    ...
    # --- Ending Routine "prac_instrucations_CB1" ---
    for thisComponent in prac_instrucations_CB1Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('prac_instrucations_CB1.stopped', globalClock.getTime())
    # check responses
    ...
    thisExp.nextEntry()
    # the Routine "prac_instrucations_CB1" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
    ...
    for thisPractice in practice:
        ...
        _resp_allKeys = []
        # keep track of which components have finished
        simon_trialComponents = [fixation_cross, left_stim, ITI, resp, right_stim]
    ...
    # keep track of which components have finished
    trial_instructionsComponents = [text_norm_2, key_instruct_2]
